...
from testglossary.main import active_API_version, app
...
def test_page_path_parameter_should_not_be_required():
    ...
    assert response_with_page.json().get("result") == response_without_page.json().get(
        "result"
    )


def test_page_path_should_be_positive():
    response_with_page_negative = client.get(
        "{}?page=-1".format(active_target_url))
...
def test_terms_per_page_path_parameter_should_not_be_required():
    ...
    response_without_terms_per_page = client.get(
        "{}".format(active_target_url))
    assert (
        response_with_terms_per_page.status_code
        == response_without_terms_per_page.status_code
    )
    assert response_with_terms_per_page.json().get(
        "result"
    ) == response_without_terms_per_page.json().get("result")
...
def test_terms_list_response_should_have_paginated_result_structure():
    ...
    assert expected_attributes == [
        ...
        if attribute in response_attributes
    ]
...
def test_search_term_with_test_term_by_page_path_parameter_should_be_greater_than_one(
    search_term_by,
):
