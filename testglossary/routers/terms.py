...
@router.get(
    path="/search/by_acronym/{acronym}",
    summary="Search by term acronym",
    description="Search by specific test term acronym",
    response_model=serializers.Paginated_terms_list,
)
async def get_term_by_acronym(
    acronym: str = Path(
        None, title="Test term query string", max_length=10, min_length=3
    ),
    page: int | None = Query(default=0, title="Page number", example=0, ge=0),
    terms_per_page: int
    | None = Query(default=5, title="Number of terms listed by page", example=10, ge=1),
    ...
    Search for a term by its name
    """
    if db_terms := interface.get_term_by_acronym(
        db, term_acronym=acronym, page=page, results_per_page=terms_per_page
    ):
        paginated_terms_list = serializers.Paginated_ouput(
            result=db_terms, page=page, offset=terms_per_page
        )
